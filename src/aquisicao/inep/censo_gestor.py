...
class GestorETL(BaseCensoEscolarETL):
    ...
    def __init__(
        ...
        self._configs = carrega_yaml("aquis_censo_gestor.yml")
    ...
    def documentos_saida(self) -> typing.List[Documento]:
        ...
        if not hasattr(self, "_documentos_saida"):
            self._documentos_saida = [
                Documento(
                    ds=self._ds,
                    referencia=dict(CatalogoAquisicao.CENSO_GESTOR),
                ),
            ]
    ...
    def ajustes_finais(self) -> None:
        ...
        """
        gestor = self._dados_saida[0].data

        # faz o sorting e reset index
        gestor.sort_values(by=["ID_GESTOR", "ANO"], inplace=True)
        ...
        gestor = gestor.reindex(columns=self._configs["DS_SCHEMA"])

        # preenche nulos com valores fixos
        for c, p in self._configs["PREENCHER_NULOS"].items():
            if c in gestor:
                gestor[c] = gestor[c].fillna(p)

        # ajusta o schema
        for c, dtype in self._configs["DS_SCHEMA"].items():
            if dtype.startswith("pd."):
                gestor[c] = gestor[c].astype(eval(dtype))
            else:
                gestor[c] = gestor[c].astype(dtype)

        self._dados_saida[0].data = gestor
